...
jest.mock('@/context/LanguageContext', () => ({ useTranslation: () => ({ currentLanguage: 'en', t: { walletPage: {}, common: { edit: 'Edit' } } }) }));
...
describe('Wallet screen boundary', () => {
  beforeEach(() => { mockPush.mockReset(); mockNavigate.mockReset(); mockUpdateCard.mockReset(); mockCards = []; mockSubscriptions = []; });
  it('keeps the Expo route as the feature-screen facade', () => expect(WalletRoute).toBe(WalletScreen));
  it('opens the add-card route from the empty state', async () => { const result = await render(<WalletScreen />); fireEvent.press(result.getByText('Add First Card')); expect(mockPush).toHaveBeenCalledWith('/(tabs)/wallet/add'); });
  it('expands a card with normalized monthly commitment and keeps edit navigation', async () => {
    ...
    fireEvent.press(result.getByRole('button', { name: 'Open Main card details' }));
    await waitFor(() => expect(result.getByText('₺100.00/mo')).toBeTruthy());
    fireEvent.press(result.getByText('Edit'));
    expect(mockNavigate).toHaveBeenCalledWith({ pathname: '/(tabs)/wallet/[id]', params: { id: 'card' } });
  });
  it('keeps pin mutation behavior', async () => { mockCards = [card()]; const result = await render(<WalletScreen />); fireEvent.press(result.getByRole('button', { name: 'Toggle pin' })); expect(mockUpdateCard).toHaveBeenCalledWith({ id: 'card', data: { isPinned: true } }); });
});
