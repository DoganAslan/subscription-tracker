...
import { ActivityIndicator, Platform, RefreshControl, ScrollView, StyleSheet, Text, TouchableOpacity, useWindowDimensions, View, type ViewStyle } from 'react-native';
...
export default function AnalyticsScreen({ initialDate }: AnalyticsScreenProps = {}) {
  ...
  const pairedCardStyle: ViewStyle = {
    flex: layout.columns === 2 ? 1 : undefined,
    width: layout.columns === 1 ? '100%' : undefined,
  };
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background, paddingTop: screenTopSpacing }]} edges={['top', 'left', 'right']}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor={colors.primary} />}>
        <ResponsiveContent testID="analytics-responsive-content" style={styles.scrollContent}>
          ...
          {analysis.activeCount === 0 ? (
            ...
            <View style={[styles.twoColumn, { flexDirection: layout.columns === 2 ? 'row' : 'column', alignItems: 'stretch' }]}>
              <SpendingBreakdownSection colors={colors} isTurkish={isTurkish} categories={topCategories} formatMoney={formatMoney} style={pairedCardStyle} />
              <UpcomingPaymentsSection colors={colors} isTurkish={isTurkish} payments={topUpcoming} upcomingTotal={analysis.upcoming30DayTotal} formatMoney={formatMoney} formatMonth={formatMonth} onOpenCalendar={() => router.push('/(tabs)/calendar')} style={pairedCardStyle} />
            </View>
...
const styles = StyleSheet.create({
  ...
  scrollView: { flex: 1 }, scrollContent: { paddingBottom: 130, gap: 16 },
  ...
  twoColumn: { gap: 16 }, disclaimer: { fontSize: 9, lineHeight: 14, textAlign: 'center', paddingHorizontal: 16 },
});
