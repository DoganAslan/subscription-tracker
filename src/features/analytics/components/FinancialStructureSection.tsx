...
function StructureItem({ width, icon, color, label, value, detail, colors }: { width: DimensionValue; icon: IconName; color: string; label: string; value: string; detail: string; colors: ThemeColors }) {
  return <View style={[styles.item, { width, backgroundColor: colors.surfaceSubtle }]}><View style={[styles.icon, { backgroundColor: `${color}1F` }]}><Ionicons name={icon} size={18} color={color} /></View><Text numberOfLines={2} style={[styles.label, { color: colors.textSecondary }]}>{label}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} style={[styles.value, { color: colors.text }]}>{value}</Text><Text numberOfLines={2} style={[styles.detail, { color: colors.textSecondary }]}>{detail}</Text></View>;
}
...
const styles = StyleSheet.create({ grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, alignItems: 'stretch' }, item: { minHeight: 132, borderRadius: 16, padding: 13 }, icon: { width: 34, height: 34, borderRadius: 11, alignItems: 'center', justifyContent: 'center', marginBottom: 10 }, label: { minHeight: 24, fontSize: 9, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.4 }, value: { fontSize: 14, fontWeight: '900', marginTop: 4 }, detail: { minHeight: 22, fontSize: 9, marginTop: 3 } });
